...
Ext.ux.form.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
        ...
        var msConfig = [{
            legend: 'Available Items',
            draggable: true,
            droppable: true,
            width: 100,
        },{
            legend: 'View in Columns',
            droppable: true,
            draggable: true,
            width: 100,
            autoHeight: true
        },{
            legend: 'View in Details',
            droppable: true,
            draggable: true,
            width: 100,
            autoHeight: true
        ...
        this.toMultiselect2 = new Ext.ux.form.MultiSelect(Ext.applyIf(this.multiselects[2], msConfig[2]));
        this.toMultiselect2.on('dblclick', this.onRowDblClick, this);
        ...
        var icons = new Ext.Panel({header:false,border:false,width:'10px'});
        p.add(icons);
        var targets = new Ext.Panel({
			type:'vbox',
			label: false,
            border:this.border,
			align:'stretchmax',
			header:false,
			items:[this.toMultiselect,this.toMultiselect2]
		});
        p.add(targets);
        p.render(this.el);
    ...
    doLayout: function(){
        if(this.rendered){
            ...
            this.toMultiselect2.fs.doLayout();
        }
    },
    ...
    afterRender: function(){
        ...
        this.toStore2 = this.toMultiselect2.store;
        this.toStore2.on('add', this.valueChanged, this);
        this.toStore2.on('remove', this.valueChanged, this);
        this.toStore2.on('load', this.valueChanged, this);
        this.valueChanged(this.toStore2);
    },
    ...
    toTop : function() {
        ...
        var selectionsArray = this.toMultiselect2.view.getSelectedIndexes();
        var records = [];
        if (selectionsArray.length > 0) {
            selectionsArray.sort();
            for (var i=0; i<selectionsArray.length; i++) {
                record = this.toMultiselect2.view.store.getAt(selectionsArray[i]);
                records.push(record);
            }
            selectionsArray = [];
            for (var i=records.length-1; i>-1; i--) {
                record = records[i];
                this.toMultiselect2.view.store.remove(record);
                this.toMultiselect2.view.store.insert(0, record);
                selectionsArray.push(((records.length - 1) - i));
            }
        }
        this.toMultiselect2.view.refresh();
        this.toMultiselect2.view.select(selectionsArray);
