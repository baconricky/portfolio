/** Inline form validation: http://www.position-absolute.com
 *  Form Input Hinting (CoolInput Plugin): http://code.alexweber.com.br/jquery/coolinput/
    ...
    $.validationEngineLanguage = {
        newLang : function() {
            $.validationEngineLanguage.allRules = {
                ...
                "onlyNumber" : {
                    "regex" : /^[\d]+$/,
                    "alertText" : "* This field should only contain numbers"
                },
                "onlyYN" : {
                    "regex" : /^[Y|N]{1}$/,
                    "alertText" : "* (Y)es or (N)o  only, please"
                },
                "capChar" : {
                    "regex" : /^[^a-z]+$/,
                    "alertText" : "* Uppercase letters only, please"
                },
                "onlyCapLetters" : {
                    "regex" : /^[A-Z\ \']+$/,
                    "alertText" : "* Uppercase letters only, please"
...
(function($) {"use strict";
    ...
    var methods = {
        ...
        _validateField : function(field, options, skipAjaxValidation) {
            ...
            for(var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    default:
                    //$.error("jQueryValidator rule "+rules[i]+" not found for "+fieldName);
                }
        ...
        _past : function(field, rules, i, options) {
            ...
            var pdate = new Date();
            //Current Date
            pdate = (p.toLowerCase() == "now") ? new Date() : methods._parseDate(p);
            //if "now", use current date, otherwise parse date passed in

            var vdate = new Date();
            //Input Date
            vdate = methods._parseDate(field.val());
            //parse date passed in
        ...
        _parseDate : function(d) {
            ...
            //setFullYear(2012,2,16);
            var dt = new Date();
            //dt.setFullYear(year         , month             , day         );
            dt.setFullYear(dateParts[2], (dateParts[0] - 1), dateParts[1]);
