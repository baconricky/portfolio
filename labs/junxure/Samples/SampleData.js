;(function($, undefined) {
    ...
    var Essentials = window.Essentials = window.Essentials || {},
        SampleData = window.SampleData = window.SampleData || {},
    ...
    SampleData.ViewByDataSource = function(schema_data) {
        var ds_url = "ERROR";
        
        if (schema_data === "actions") {
            ds_url = "/Samples/ViewByAction.json";
        } else if (schema_data === "records") {
            ds_url = "/Samples/ViewByRecord.json";
    ...
    SampleData.GetSidebarData = function() {
        return [{
                text: "Records",
                url: "records.html"
            },{
    ...
    SampleData.Actions = function(count) {
        var arr = [],
            count = count || 5,
            now = new Date(),
            i=0;
    ...
    SampleData.GetRecordData = function(count) {
        ...
        for (i = 0; i < count; i++) {
            var ns = new Nonsense(i),
                ...
                email = ns.firstName() + "." + ns.lastName() + "@aol.com",
                priAdv = ns.name(),
                priContact = ns.name(),
                priContactBday = BirthDates[Math.floor(Math.random() * BirthDates.length)],
                priContactEmail = ns.firstName() + "." + ns.lastName() + "@aol.com",
                priContactPhone = "(919) 555-1212",
                secAdv = ns.name(),
                secContact = ns.name(),
                secContactBday = BirthDates[Math.floor(Math.random() * BirthDates.length)],
                secContactEmail = ns.firstName() + "." + ns.lastName() + "@aol.com",
                secContactPhone = "(919) 555-1212",
                csr = ns.name(),
        ...
        /**
        _ActionEditorDiv = "#app-action-editor",
        _ActionEditorList = function() {
            setTimeout(function() {
                var header = kendo.template($("#detail-header-template").html());

                $("#detail-header").html( header );
                
                if ($(_ActionEditorDiv).length) {
                    $(_ActionEditorDiv).kendoListView({
                        dataSource: {
                            data: jxSampleData._ActionList(10),
                            pageSize: 20
                        },
