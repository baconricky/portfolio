...
var STATE_OPTIONS_FORM = 'https://engage.redhat.com/forms/System-State';  // a utility form which defines the list of states
...
var ELOQUA_PREPOPULATE_FIELDS = [
	...
	['PrimaryJobFunction', 'C_Job_Role1']
];
...
var utils = {
	...
	getStateOptions : function(code) {
		...
		catch (e) { // probably a permission denied / iframe problem
			return false;
		}
	...
	initStateOptions : function() {
		...
		var itvl = window.setInterval(function(){
			if (++i >= 100) { // looped long enough...
				window.clearInterval(itvl);
				return;
			}
			if (that.getStateOptions() !== false) {
				that.updateState();
				window.clearInterval(itvl);
			}
		}, 100);
...
EloquaHelper = function(cfg) {
	...
	return {
		...
		init : function() {
			...
			if (typeof cfg.customValidators == 'object') {
				for (var i=0; i<cfg.customValidators.length; i++) {
					this.addCustomValidation(cfg.customValidators[i]);
				}
			}
		...
		overrideValidation : function() {
			...
			// overrides a built-in Eloqua validation function
    			...
    			// overrides a built-in Eloqua validation function
    			ResetHighlight = function(clearErrors) {
       				var field;
