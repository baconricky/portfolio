...
/**
 * Added by RH GIT, hack for localization
 */
...
ModuleControlBlocks.prototype.injectFrames = function() {
  for (var i = 0, n = this.blocks_.length; i < n; ++i) {
    ...
    var selectorUrl = mcb.getBaseURL() + mcb.getName() + ".nocache.htm";

    if(isI18n()){
     	var language = 'en';

    	var platform = window["prop$user.agent"]();
     	switch (platform){
       		case "ie6":
     ...
     var splitlanguage = language.split('-')[0];

     if (hasLanguage(splitlanguage)) {
         selectorUrl = mcb.getBaseURL() + mcb.getName() + "_" + splitlanguage +  ".nocache.htm";
     }else{
	 splitlanguage = 'en';
         selectorUrl = mcb.getBaseURL() + mcb.getName() + "_" + splitlanguage +  ".nocache.htm";
     }
