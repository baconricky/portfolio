...
//@exports([loadRedBox, playVideo, closeRedBox, resizeVideo])
//@assumes access to window and document objects in browser


/*****
EXPECTS THE FOLLOWING MARKUP IDIOM:
<a class="video" href="url/of/video?ar=WxH">
  <img class="video-still" src="url/of/still" alt="description of video" title="title of video" width="..." height="..." />
</a>
...
var getDimensions = function(aspectRatio){
  ...
  switch (aspectRatio) {
    case '16x9':
      dims.maxWidth = 656; // redesigned site max in-page size (656x369px)
    ...
    default:
      ...
      dims.minHeight = 297 + pHeight(dims.minWidth);
      break;    
  }
  return dims;
}

//@constructor for the RedBox object for videos
function RedBox2(title, filepath, preview, aspectRatio) {
  var playerURL = '/v/swf/redbox/redbox-player.swf';
...
RedBox2.prototype.toDOM = function(document, s_account) {
  ...
  redBoxClose.onclick = function() { closeRedBox(); return false; };
  ...
  // we can overwrite the default handler since this is a transient DOM subtree
  rBB.onclick = function() { closeRedBox(); return false; };
...
var clickClose = function() { closeRedBox(); return false; };
...
function loadRedBox2(document, omnitureAccount) {
...
function playVideo(videoLink) {
  ...
  var curID       = videoLink.id,
      ...
      stillFrame  = $(videoLink).children($('img.video-still'))[0],
  ...
  newVideo = new RedBox2(stillFrame.title, videoLink.href.split('?')[0], stillFrame.src, aspectRatio);
  ...
  rBC = $('#redBoxClose')[0];
  rBB = $('#redBoxBg')[0];
  rBC.onclick, rBB.onclick = function(){ closeRedBox(scrollCoord.scrollY); return false; };
