...
var SANDBOX = window.location.host != 'www.redhat.com';
...
var utils = {
	...
	// TODO: make this work for other layouts besides default Eloqua layout
	...
	prepopulateFromLookup : function(lookup, fieldsToHideIfPopulated) {
		for (var i=0; i<ELOQUA_PREPOPULATE_FIELDS.length; i++) {
			...
			utils.popField(field, value, fieldsToHideIfPopulated && fieldsToHideIfPopulated.indexOf(field) != -1);
			utils.popField(cField, value, fieldsToHideIfPopulated && fieldsToHideIfPopulated.indexOf(cField) != -1);
		}
	},
...
TacticOfferCampaignTrackingStrategy = function() {
	...
	return {
		fetchCampaignIds : function(form, callback) {
			...
			var campaignLookup = EloquaDataLookupHelper.buildLookup(1798, 'da39f645-802e-48c3-a690-bf1c4d694727', 'HTML_Form_Name1', formName, function(){
		...
		overrideValidation : function() {
			...
			// overrides a built-in Eloqua validation function
    			DisplayErrorSet = function(ErrorSet) {
       				...
       				if (ErrorSet.length && document.getElementById( utils.errorMessageId )) {
					utils.displayErrorMessage(errorMessageText);
       				}
    			}
